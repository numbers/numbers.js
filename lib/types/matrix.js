 ...
 */
Matrix.prototype.multiply = function (arr) {
  if (this.data[0].length === arr.data.length) {
    var result = new Array(this.data.length);
    
    for (var x = 0 ; x < this.data.length ; x++) {
      result[x] = new Array(arr.data[0].length);
    }

    var t = arr.clone().transpose();
    
    for (var i = 0 ; i < result.length ; i++) {
      for (var j = 0 ; j < result[i].length ; j++) {
        result[i][j] = matrix.dotproduct(this.data[i],t.data[j]);
      }
    }
...
Matrix.prototype.rotate = function (degree, direction) {
  if (this.data.length === 2) {
    ...
    this.previousTransformation.push(transformation.data);
...
Matrix.prototype.undo = function () {
  if (this.previousTransformation.length > 0) {
    var inverted = new Matrix(this.previousTransformation.pop()).invert();
