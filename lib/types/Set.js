...
function Set(data, useCache) {
  var set = numbers.util.parasiticExtend(
              //base object is an array
              [],
              
              //the methods to expose
              [has, size, add, clone, toPlainArray,
  ...
  //add delete separately (thanks reserved word policy)
  numbers.util.parasiticAddRenamedMethod(set, remove, 'delete', Set);
  numbers.util.parasiticAddRenamedMethod(set, add, 'push', Set);
...
Set.isSet = function(obj) {
  return numbers.util.parasiticIs(obj, Set);
}
...
function toPlainArray() {
  for(var method in Set._typeMethods) {
    delete this[method];
  }
