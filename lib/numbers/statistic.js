 ...
 */
statistic.variance = function (arr) {
  var count = arr.length;
  var mean = statistic.mean(arr);
  var value = 0;
  for (var i = 0; i < count; i++) {
    value += Math.pow((arr[i] - mean), 2);
  }
  return value / count;
};

/**
 * kurtosis calculates "excess kurtosis" also know as "Pearson's kurtosis", ie. a normal 
 * distribution will have kurtosis = 0
 * https://en.wikipedia.org/wiki/Kurtosis
 * 
 * @param  {Array} arr    The array of values
 * @return {Number}       The kurtosis
 */
statistic.kurtosis = function (arr) {
  var count = arr.length;
  var mean = statistic.mean(arr);
  var s2 = 0;
  var s4 = 0;
 ...
 */
statistic.skewness = function (arr) {
  var count = arr.length;
  var variance = statistic.variance(arr);
  var mean = statistic.mean(arr);
  var value = 0;
  for (var i = 0; i < count; i++) {
    var d = arr[i] - mean;
  ...
  value = value / (variance * Math.sqrt(variance));
