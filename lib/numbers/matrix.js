 ...
 * 8 possible permutations for this.
 *
...
matrix.zigzag = function(n, point, dir) {
  ...
  BRH = function(M) { //starting at bottom right, moving horizontally
    var jump = false, tl = n*n, br = 1, inc = 1;
    M[0][0] = tl;
    M[n-1][n-1] = br;
  ...
  BRV = function(M) {//starting at bottom right, moving vertically
    return matrix.transpose(BRH(M));
  }

  BLH = function(M) {//starting at bottom left, moving horizontally
    return matrix.reverseCols(BRH(M));
  }

  BLV = function(M) {//starting at bottom left, moving vertically
