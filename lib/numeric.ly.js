...
var numeric = {
  ...
  addition: function(arr) {
    if (Object.prototype.toString.call(arr) === '[object Array]') {
    	//use already available method in javascript
  ...
  // draws a random quantity of elements from an array
  random: function(arr, quant) {
    if (arr.length <= quant){
      throw new Error('Quantity requested exceeds size of array');
    } else if (arr.length === 0){
      throw new Error('Empty array');
    } else {
      return numeric.shuffle(arr).slice(0, quant);
    }
  },
...
/*-----------------------------------------------------------------------------*/
  ...
  prime: {
    ...
    // return prime factorization of a number (return as an array of integers)
    // uses trial division
